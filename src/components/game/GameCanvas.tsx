...
interface GameCanvasProps {
  ...
  onGameStatsUpdate?: (stats: { collectedCoins: number; totalCoinsOnLevel: number; }) => void;
}
...
export default function GameCanvas({
  ...
  const spawnSingleCoin = useCallback(
    (currentProcessedLevel: ProcessedLevelData, currentCanvasWidth: number, currentCanvasHeight: number): CoinState[] => {
        ...
        const uniqueId = `coin-${Date.now()}-${totalCoinsSpawnedThisLevelRef.current + 1}`;
  ...
  useEffect(() => {
    const allAssetsLoaded = assets.playerImageLoaded && assets.tileImageLoaded && assets.coinImageLoaded &&
                            ...
                            assets.flowersImageLoaded && assets.largeBushImageLoaded && assets.houseImageLoaded &&
  ...
  }, [isClient, rawLevelData, canvasSize, assets, processRawLevelData, parentPlayerRef, setProcessedLevel]); 

  // Effect 6: Update refs for states
  ...
  useEffect(() => {
    ...
    if (!coinsAttemptedOrPresent && processedLevelRef.current.tiles.length > 0 && totalCoinsSpawnedThisLevelRef.current < TOTAL_COINS_ON_LEVEL) {
        ...
        setActiveCoins(newCoins); // spawnSingleCoin is one coin
    ...
    isLoading, isClient, canvasSize, levelPath, assets, processedLevel, 
    spawnSingleCoin, spawnSingleEnemy, setActiveCoins, setActiveEnemies, setIsLoading
  ]);
  ...
  useEffect(() => {
    ...
    if (currentActiveCoins.length === 0 && totalCoinsSpawnedThisLevelRef.current < TOTAL_COINS_ON_LEVEL) {
        ...
        setTotalCoinsSpawnedThisLevel(prevCount => prevCount + currentActiveCoins.length); 
      ...
      activeCoins, isLoading, isClient, canvasSize, processedLevel, 
      TOTAL_COINS_ON_LEVEL, spawnSingleCoin, setActiveCoins, setTotalCoinsSpawnedThisLevel,
  ]);
    ...
    if (player) {
        ...
        if (player.isOnGround && activePlatformThisFrame) {
            ...
            if (currentPlatform.id === 'p3' && p3Tile && p3MovementStateRef?.current) { 
                 ...
                 const currentElapsedTime = Date.now() - p3_start_time;                       
