...
export default function GameCanvas({ levelPath, onPlayerAction, playerRef: parentPlayerRef, executeAction, resetExecuteAction }: GameCanvasProps) {
  // console.log(`[GameCanvas] Component body. levelPath: ${levelPath}`); // Basic log
  ...
  const spawnNewCoinPair = useCallback(
    (currentProcessedLevel: ProcessedLevelData | null, currentCanvasWidth: number, currentCanvasHeight: number): CoinState[] => {
        ...
        if (ySpawnZoneTopCoinTopEdge >= ySpawnZoneBottomCoinTopEdge) {
            // console.warn(`[spawnNewCoinPair] Invalid spawn zone: Top edge (${ySpawnZoneTopCoinTopEdge.toFixed(2)}) is above or at bottom edge (${ySpawnZoneBottomCoinTopEdge.toFixed(2)}). No coins spawned.`);
            return [];
        }
  ...
  const spawnSingleEnemy = useCallback(
    (currentProcessedLevel: ProcessedLevelData | null, currentCanvasWidth: number): EnemyState | null => {
      ...
      if (!p1 || !p2) {
        // console.warn(`[spawnSingleEnemy] P1 or P2 not found for level ${levelPath}. Cannot spawn enemy.`);
        return null;
      }
      ...
      // console.log("[spawnSingleEnemy] Spawned new enemy:", newEnemy);
      return newEnemy;
    },
  ...
  useEffect(() => {
    ...
    const smallBushImg = new Image(); smallBushImg.src = `/assets/images/flowers.png`; smallBushImg.setAttribute('data-ai-hint', 'flowers small bush');
  ...
  useEffect(() => {
    ...
    if (!isClient || !rawLevelData || canvasSize.width === 0 || canvasSize.height === 0 || !allAssetsLoaded) {
        if (!isLoading) setIsLoading(true);
        setProcessedLevel(null); // Clear processed level if critical data is missing
  ...
  // Effect 6: (Removed - Ref update is now in Effect 5)

  // Effect 7: Spawn entities and finish loading
  useEffect(() => {
    console.log(`[GameCanvas Effect 7] Running. isLoading: ${isLoading}, isClient: ${isClient}, processedLevelRef.current: ${!!processedLevelRef.current}, canvasSize W:${canvasSize.width}H:${canvasSize.height}, levelPath: ${levelPath}`);

    if (!isLoading) { // This effect should only run if we are in a loading state
    ...
    if (!isClient || !processedLevelRef.current || canvasSize.width === 0 || canvasSize.height === 0) {
      // console.log("[GameCanvas Effect 7] Pre-conditions (isClient, processedLevelRef.current, canvasSize) not met, returning.");
      return;
    }
    ...
    if (coinsSpawnedOrAttempted && enemiesSpawnedOrAttempted) {
      ...
      isClient, isLoading, processedLevelRef, canvasSize, levelPath, activeCoins.length, activeEnemies.length, // Keep these to re-evaluate if entities disappear
      spawnNewCoinPair, spawnSingleEnemy, setActiveCoins, setActiveEnemies, setIsLoading
  ]);

  // Effect 8: Respawn coins
   useEffect(() => {
    // console.log(`[GameCanvas Effect 8] Running: Check coin respawn. isLoading: ${isLoading}, processedLevelRef.current: ${!!processedLevelRef.current}, activeCoins: ${activeCoins.length}`);
  ...
  const gameLoop = useCallback(() => {
    // console.log("[gameLoop] CALLED"); // Basic log to confirm loop is running
    const canvas = canvasRef.current;
    ...
    if (currentExecuteActionVal) {
      ...
      switch (currentExecuteActionVal) {
        ...
        case 'jump':
          // console.log(`[gameLoop] Attempting jump. player.isOnGround: ${player.isOnGround}`);
    ...
    player.vx = 0;
    if (player.isMovingLeft) { player.vx = -PLAYER_SPEED; player.facingDirection = 'left'; }
    if (player.isMovingRight) { player.vx = PLAYER_SPEED; player.facingDirection = 'right'; }
    ...
    if (p3Tile && p3BasePosition.current && p3InterestPoints.current.length > 0) {
        if (!p3MovementStateRef.current) {
            ...
            p3CurrentTargetIndex.current = (nextTargetIndex === p3CurrentTargetIndex.current && p3InterestPoints.current.length > 1) ? (nextTargetIndex + 1) % p3InterestPoints.current.length : nextTargetIndex;
            ...
            p3MovementStateRef.current = {
                ...
                targetX: p3BasePosition.current.x + targetOffset.x, targetY: p3BasePosition.current.y + targetOffset.y,
            };
        }

        const state = p3MovementStateRef.current;
        const elapsedTime = loopStartTime - state.startTime;
        let t = Math.min(elapsedTime / P3_MOVEMENT_DURATION, 1);
    ...
    updatedTiles.forEach(tile => {
      if (tile.type === 1 && checkCollision({ ...player, y: proposedY }, tile)) {
        if (player.vy > 0) {
          ...
          if (tile.id === 'p3') { platformInducedMoveX = p3_delta_x; player.y += p3_delta_y; proposedY = player.y; onMovingPlatform = true; }
        } else if (player.vy < 0) { proposedY = tile.y + tile.height; player.vy = 0; }
      }
    });
    player.y = proposedY;

    if (onMovingPlatform && activePlatformId !== 'p3') proposedX += platformInducedMoveX;
    else if (onMovingPlatform && activePlatformId === 'p3') proposedX += p3_delta_x;

    updatedTiles.forEach(tile => {
      if (tile.type === 1 && checkCollision({ ...player, x: proposedX }, tile)) {
        if (player.vx > 0) proposedX = tile.x - player.width;
        else if (player.vx < 0) proposedX = tile.x + tile.width;
        player.vx = 0;
      }
    });
    player.x = proposedX;
    ...
    setActiveCoins(prevCoins => prevCoins.map(coin => {
        ...
        if (!newCoin.isCollected && newCoin.currentOpacity > 0.5 && checkCollision(player, newCoin)) {
            newCoin.isCollected = true; newCoin.currentOpacity = 0;
    ...
    setActiveEnemies(prevEnemies => prevEnemies.map(enemy => {
      ...
      if (checkCollision(player, updatedEnemy)) {
        ...
        player.x = ground ? canvas.width / 2 - player.width / 2 : canvas.width / 2 - player.width / 2;
        player.y = ground ? ground.y - player.height : canvas.height - player.height - 50;
    ...
    // console.log(`[gameLoop] Drawn red square at ${canvas.width / 2 - 25}, ${canvas.height / 2 - 25}`);


    // Commented out original rendering to simplify for debugging
    /*
    updatedTiles.forEach(tile => {
      if (tile.layer !== 'foreground') {
        if (tile.type === 1) {
          if (tile.id.startsWith('stone_') && assets.stoneImage?.complete) ctx.drawImage(assets.stoneImage, tile.x, tile.y, tile.width, tile.height);
          else if (assets.tileImage?.complete) ctx.drawImage(assets.tileImage, tile.x, tile.y, tile.width, tile.height);
          ...
          else if (tile.id !== 'water_area' && tile.id !== 'water_pit') { ctx.fillStyle = tile.color; ctx.fillRect(tile.x, tile.y, tile.width, tile.height); }
        }
      }
    });

    const waterTile = updatedTiles.find(t => t.id === 'water_area' || t.id === 'water_pit');
    if (waterTile) { ctx.fillStyle = waterTile.color; ctx.fillRect(waterTile.x, waterTile.y, waterTile.width, waterTile.height); }
    ...
    updatedTiles.forEach(tile => {
      if (tile.layer === 'foreground') {
        ...
        else if (tile.id === "bush1" && assets.flowerImage?.complete) ctx.drawImage(assets.flowerImage, tile.x, tile.y, tile.width, tile.height);
        else { ctx.fillStyle = tile.color; ctx.fillRect(tile.x, tile.y, tile.width, tile.height); }
      }
    });
    */
    ...
    isClient, assets, resetExecuteAction, setActiveCoins, setActiveEnemies, canvasSize, levelPath,
    GRAVITY, PLAYER_SPEED, JUMP_STRENGTH, PLAYER_WIDTH, PLAYER_HEIGHT, PLAYER_COLOR,
  ...
  }, [isClient, isLoading, canvasSize, processedLevelRef, gameLoop]); // Added processedLevelRef to dependencies for re-evaluation
