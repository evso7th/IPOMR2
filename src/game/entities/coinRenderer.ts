...
import { COIN_COLOR, COIN_SHADOW_COLOR, COIN_SHADOW_BLUR, COIN_SHADOW_OFFSET_X, COIN_SHADOW_OFFSET_Y, COIN_ROTATION_SPEED_MAX, COIN_SIZE } from '@/config/gameConfig';
...
export const renderCoins = (
  ...
  if (!coins || coins.length === 0) {
    // console.log("[renderCoins] No coins to render or coins array is null.");
    return;
  }
  ...
  coins.forEach((coin, index) => {
    ...
    } else if (!coin.isCollected && coin.currentOpacity > 0.5 && coin.isVisuallyPresent) { 
        // Simplified diagnostic drawing: bright yellow square
        ctx.fillStyle = 'yellow';
        ctx.fillRect(coin.x, coin.y, COIN_SIZE, COIN_SIZE);
        ctx.strokeStyle = 'orange';
        ctx.lineWidth = 2;
        ctx.strokeRect(coin.x, coin.y, COIN_SIZE, COIN_SIZE);
